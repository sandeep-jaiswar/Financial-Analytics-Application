...
# Market Data API Testing Script
# This script demonstrates how to test the market data API endpoints
# Prerequisites: Django server running and ClickHouse populated with data

BASE_URL="http://localhost:8000"
API_URL="${BASE_URL}/api/marketdata"

echo "========================================"
echo "Market Data API Testing Script"
echo "========================================"
echo ""
...
echo -e "${GREEN}1. Get All Symbols${NC}"
test_endpoint "List all symbols" "${API_URL}/symbols/"
...
# 3. Test market data for a specific symbol (default date range)
echo -e "${GREEN}3. Get Market Data for Symbol (Default Dates)${NC}"
test_endpoint "AAPL market data (last 30 days)" "${API_URL}/AAPL/"

# 4. Test market data with date range
echo -e "${GREEN}4. Get Market Data with Date Range${NC}"
FROM_DATE=$(date -d "30 days ago" +%Y-%m-%d)
TO_DATE=$(date +%Y-%m-%d)
test_endpoint "AAPL from ${FROM_DATE} to ${TO_DATE}" "${API_URL}/AAPL/?from=${FROM_DATE}&to=${TO_DATE}"

# 5. Test market data with pagination
echo -e "${GREEN}5. Get Market Data with Pagination${NC}"
test_endpoint "AAPL page 1, 10 per page" "${API_URL}/AAPL/?from=${FROM_DATE}&to=${TO_DATE}&page=1&per_page=10"
...
# 7. Test latest market data for specific symbols
echo -e "${GREEN}7. Get Latest Market Data (Specific Symbols)${NC}"
test_endpoint "Latest data for AAPL,GOOGL,MSFT" "${API_URL}/latest/?symbols=AAPL,GOOGL,MSFT"
...
# 9. Test error handling - invalid date format
echo -e "${GREEN}9. Test Error Handling - Invalid Date${NC}"
echo -e "${BLUE}Testing: Invalid date format${NC}"
echo -e "${YELLOW}curl ${API_URL}/AAPL/?from=invalid-date${NC}"
RESPONSE=$(curl -s "${API_URL}/AAPL/?from=invalid-date")
echo "$RESPONSE" | jq '.'
...
# 10. Test error handling - invalid date range
echo -e "${GREEN}10. Test Error Handling - Invalid Date Range${NC}"
echo -e "${BLUE}Testing: From date after to date${NC}"
echo -e "${YELLOW}curl ${API_URL}/AAPL/?from=2024-12-31&to=2024-01-01${NC}"
RESPONSE=$(curl -s "${API_URL}/AAPL/?from=2024-12-31&to=2024-01-01")
echo "$RESPONSE" | jq '.'
...
echo ""
echo "Summary:"
echo "- Symbols endpoint: GET /api/marketdata/symbols/"
echo "- Market data by symbol: GET /api/marketdata/:symbol/"
echo "- Latest market data: GET /api/marketdata/latest/"
echo ""
